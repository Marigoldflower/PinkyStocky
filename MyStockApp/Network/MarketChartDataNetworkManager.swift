...
enum MarketDataNetworkError: Error {
...
final class MarketDataNetworkManager {
    
    // 여러 뷰 컨트롤러에서 통신을 한다면, 일반적으로 싱글톤 패턴으로 만든다.
    static let shared = MarketDataNetworkManager()
    ...
    typealias NetworkCompletion = (Result<[MarketData], MarketDataNetworkError>) -> Void
    ...
    func fetchNetwork(searchTerm: String, completion: @escaping NetworkCompletion) {
        ...
        let urlString = "https://finnhub.io/api/v1/stock/candle?symbol=\(searchTerm)&resolution=1&from=\(oneYearsAgo)&to=\(today)&token=\(myAPIKey)"
        print(urlString)
    ...
    private func parseJSON(_ marketData: Data) -> [MarketData]? {
        //print(#function)
        ...
        do {
            ...
            let marketData = try JSONDecoder().decode(MarketData.self, from: marketData)
            return [marketData]
        // 실패
        } catch {
