...
final class TopNewsDataNetworkManager {
    
    // 여러 뷰 컨트롤러에서 통신을 한다면, 일반적으로 싱글톤 패턴으로 만든다.
    static let shared = TopNewsDataNetworkManager()
    ...
    typealias NetworkCompletion = (Result<[TopNewsData], TopNewsError>) -> Void
    ...
    private func parseJSON(_ newsData: Data) -> [TopNewsData]? {
        //print(#function)
        ...
        do {
            ...
            let newsData = try JSONDecoder().decode(TopNews.self, from: newsData)
            return newsData.topNewsData
            // 실패
        } catch {
            print(error)
